/****************************************************************************
 *                       MVM Ventialotor V3 Firmware
 *                       
 * Open source code agreement.
 * 
 * This code is designed to run on the HW V3 of MVM Ventialor
 * 
...
void onTimerCoreTask()
{
        ...
        if (core_config.run) {
            ...
            if (core_config.BreathMode == M_BREATH_FORCED) {
                ...
                if (core_config.BreathMode == M_BREATH_ASSISTED) {
                    core_config.target_pressure = core_config.target_pressure_assist;
                    pres_peak = 0;
        ...
        if (core_sm_context.timer1 >= (core_config.inhale_ms / TIMERCORE_INTERVAL_MS)) {
            if ((core_config.pause_inhale == false) && (core_config.pause_lg == false)) {
                ...
                if (core_config.pause_lg == false)
                    ...
                    core_config.target_pressure = core_config.pause_lg_p;
                    valve_contol(VALVE_IN, VALVE_OPEN);
    ...
    if (core_config.puase_timeout > 0)
    {
      core_config.puase_timeout--;
...
void SetCommandCallback(cmd* c)
{
    ...
    if (strPatam == "pause_inhale") {
        ...
        core_config.puase_timeout=500/TIMERCORE_INTERVAL_MS;
        Serial.println("valore=OK");
    }
    ...
    if (strPatam == "pause_exhale") {
        ...
        core_config.puase_timeout=50*TIMERCORE_INTERVAL_MS;
        Serial.println("valore=OK");
    }
...
void GetCommandCallback(cmd* c)
{
    ...
    if (strPatam == "tidal") {
        Serial.println("valore=" + String(tidal_volume_c.TidalVolume*VOL_COMP));
    }
